...
<form name="assignform" method="post" action="admin.php?action=massassign">
	...
	{section name=proj loop=$projects}
		<label for="projects{$projects[proj].ID}">{$projects[proj].name}:</label>
		<input type="checkbox" value="{$projects[proj].ID}" id="projects{$projects[proj].ID}" name="projects[]" {if $projects[proj].assigned == 1} checked {/if} /><br />
	{/section}
